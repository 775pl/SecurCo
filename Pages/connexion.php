...
if(isset($_POST['submit'])){
    if(!empty($_POST['pseudo']) AND !empty($_POST['mdp'])){
            $ldap_host = "ldap://IP";
            $base_dn = "CN=Administrateur, CN=Users, DC=, DC=net";
            $pseudo_ldap = $_POST["pseudo"];
            $pseudo_dn = '' . "\\" . $pseudo_ldap;
            $mdp_ldap = $_POST["mdp"];
            $ldapPort = 389; 
        ...
        $verifications = mysqli_query($mysqli,'SELECT * FROM user WHERE pseudo = \''.mysqli_real_escape_string($mysqli, $_POST['pseudo']).'\' ');
          $data_verif = mysqli_fetch_assoc($verifications);
            ...
            elseif ( $bind == TRUE ){
                ...
                $result = ldap_search($connect, "DC=, DC=net", $filter);
                ...
                $data = ldap_get_entries($connect, $result);
                    ...
                    if(isset($data_verif['navigateur']) == $browser){
                        $details = file_get_contents("https://ipinfo.io/".$_SERVER['REMOTE_ADDR']."?token=?");
    ...
    Suite à une récente connexion sur votre compte Le Chatelet, nous avons constaté une activité suspecte. La connexion s'est opérée depuis un nouveau navigateur.";
                                    ...
                                    $dbh = new PDO('mysql:host=localhost;dbname=Users', '', 'mdp');
                                    ...
                                    $message='Afin de valider votre compte merci de cliquer sur ce lien https://domain.fr?pseudo='.urlencode($pseudo).'&key_confirm='.urlencode($key);
